# Ranxware V4 (DEMO Version)

*readme still wip*

> [!important]
> Ranxware V4 is going through major development changes! These can include:
> * One-Checkpoint Key System _(Valid for 1 Day)_
> * Script Loader Gui _(Game Detection & Script Loader)_
> * More Upcoming/Unreleased Features in **FLICK**
> * **Murderers VS Sherrifs Duels** Script Pre-release

> [!caution]
> **PLEASE USE A _HIGH-END EXECUTOR_ (e.g. [Zenith](https://zenith.win), [Wave](https://getwave.gg), [Volcano](https://volcano.wtf), etc.) AS THE SCRIPT WILL BREAK, CAUSE ERRORS, AND MAY POTENTIALLY LEAD TO ANTICHEAT DETECTIONS!**

**Get Started ⤵️**
```luau
loadstring(game:HttpGet("https://github.com/Ranxware/RanxwareV4/raw/refs/heads/main/loader.lua"))()
```

---

## Overview

The official revival of Ranxware but with more performant, utilized and optimal features.

**Ranxware V4** is designed for both closet & blatant cheaters. Ranxware also includes effective technology which bypasses anti-cheats within its supporting games.

## Demo Status

Ranxware V4 is currently in a `Demo` state, meaning that many more features and games are yet to be implemented.

> [!note]
> This also means that some features from the demo may change throughout development.

## Supported Games

* [Flick](https://www.roblox.com/games/136801880565837/)

## Upcoming Games (Demo)

* [Murderers Vs Sheriffs Duels](https://www.roblox.com/games/12355337193/)
* [Untitled Tag Game](https://www.roblox.com/games/14044547200/)

## Upcoming Games

* [Hide or Die](https://www.roblox.com/games/18799085098/)
* [Murderers Vs Sheriffs](https://www.roblox.com/games/5154858502/)
* [Dingus](https://www.roblox.com/games/13924946576/)
* [\[DUELS\] Murderers vs Sheriffs](https://www.roblox.com/games/135856908115931/)
* [Murderers Vs Sheriffs 2](https://www.roblox.com/games/13429790955/)
* [Word Bomb](https://www.roblox.com/games/2653064683/)
* [Hypershot](https://www.roblox.com/games/17516596118/)
